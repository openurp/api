...
class PlanAuditSkipListener extends PlanAuditListener {
  ...
  def startGroupAudit(context: PlanAuditContext, courseGroup: CourseGroup, groupResult: GroupAuditResult): Boolean = {
    ...
    if (standard.isDisaudit(courseGroup.courseType)) {
      ...
      val oriCreditsRequired = context.result.auditStat.requiredCredits
      val oriNumRequired = context.result.auditStat.requiredCount
      context.result.auditStat.requiredCredits = (oriCreditsRequired - courseGroup.credits)
      context.result.auditStat.requiredCount = (oriNumRequired - courseGroup.courseCount)
      return false
    }
    true
  }
